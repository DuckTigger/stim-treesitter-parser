...
local function run_tests()
	...
	test("no DETECTOR in selection returns nil, nil", function()
		...
		assert(n_meas == nil and n_det == nil, "expected nil,nil got " .. tostring(n_meas) .. "," .. tostring(n_det))
	end)
	...
	test("records below threshold are shifted", function()
		...
		assert(lines[1] == "DETECTOR rec[-7] rec[-3]", "got: " .. lines[1])
	end)
	...
	test("multiple records on one line all processed", function()
		...
		assert(lines[1] == "DETECTOR rec[-11] rec[-10] rec[-9] rec[-2] rec[-1]", "got: " .. lines[1])
	end)
