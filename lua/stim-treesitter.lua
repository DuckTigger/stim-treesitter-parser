			...
			if idx == target_index then
				if measurement.target then
					...
					vim.api.nvim_buf_add_highlight(
						bufnr,
						ns_id,
						"StimMeasurementHighlight",
						measurement.target.start_row,
						measurement.target.start_col,
						measurement.target.end_col
					)
				else
...
function M.shift_records(line1, line2)
	...
	if n_meas ~= nil and n_det ~= nil then
		...
		if n_meas ~= n_det then
			...
			threshold_prompt =
				string.format("Threshold [default -%d (%d measurements = %d detectors)]: ", n_det, n_meas, n_det)
		end
	else
	...
	vim.ui.input(
		...
		function(threshold_str)
			if threshold_str == nil then
				return
			end -- user cancelled with <Esc>
			local threshold
			...
			vim.ui.input({ prompt = "Shift amount (subtracted from index, e.g. 2): " }, function(shift_str)
				if not shift_str or shift_str == "" then
					return
				end
...
local function parse_qubit_coords(bufnr)
	...
	for id, node in query:iter_captures(root, bufnr, 0, -1) do
		...
		if coords_node and qubit_node then
			...
			local coords_text =
				vim.api.nvim_buf_get_text(bufnr, coords_start_row, coords_start_col, coords_end_row, coords_end_col, {})

			-- Extract qubit number
			local qubit_start_row, qubit_start_col, qubit_end_row, qubit_end_col = qubit_node:range()
			local qubit_text =
				vim.api.nvim_buf_get_text(bufnr, qubit_start_row, qubit_start_col, qubit_end_row, qubit_end_col, {})
				...
				if ok then
					for id, node in query:iter_captures(root, bufnr, 0, -1) do
						...
						if
							row >= start_row
							and row <= end_row
							and (row > start_row or col >= start_col)
							and (row < end_row or col < end_col)
						then
